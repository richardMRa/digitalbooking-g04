...
import { Link, parsePath, useLocation, useNavigate, useParams, useSearchParams } from "react-router-dom";
...
export default function CreateProduct(props) {
  ...
  useEffect(()=>{
    if(paramsData.idProduct){
      if(!editProductData){
        fetch(`${url.api}/api/product/${paramsData.idProduct}`)
        .then(response => response.json())
        .then(response => setEditProductData(response))
      }
      
    }
},[paramsData, editProductData])
  ...
  const handleChecked = (item)=>{
    let flag = false
    editProductData.features.forEach((e)=>{
      if(item.name === e.name){
        flag = true
      } else {
        flag = false
            ...
            <>
              <div className="top-container">
                <div className="top">
                  ...
                  <div className="back-home" onClick={handleBackHome}>
                      ...
                      <div className="form-container">
                        ...
                        <div className="field introduction">
                          ...
                          <Textarea
                            id="introduction"
                            name="introduction"
                            value={editProductData.introduction}
                      ...
                      <div className="img-container">
                        {images.length < 5 && applyValidations ? (
                          <div className="alert-images alert-danger" role="alert">
                            {images.length > 0
                              ? `Agregue 5 fotos minimo (${5 - images.length
                              } fotos más)`
                              : "Este campo es obligatorio!"}
                          </div>
                        ) : (
